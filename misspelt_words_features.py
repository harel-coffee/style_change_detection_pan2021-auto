...
def tweet_abbreviations(file_path):
    ##https://preply.com/en/blog/2020/05/07/the-most-used-internet-abbreviations-for-texting-and-tweeting/#scroll-to-heading-0
...
def internet_abbreviations(file):
...
def create_misspellings_dict(data_dir):
    dictionary = {
        ...
        'tweet_abbrevs': set(tweet_abbreviations(os.path.join(data_dir, 'tweet_abbrevs.txt'))),
        'internet_abbrevs': set(determiner(os.path.join(data_dir, 'internet_abbrevs.txt')))
    }
    return dictionary
...
class MisspellingsFeatureTransformer(BaseEstimator, TransformerMixin):
    ...
    def _process(self, x):
        if len(x['tokens']) == 0:
            return [0] * len(self.misspelling_dict)
